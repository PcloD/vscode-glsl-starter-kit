...
void main(void) {
  ...
  vec2 pos = (uv - vec2(0.5)) / vec2(upscale) + vec2(0.5);
  ...
  for(float i = 0.0; i < MAXSTEPS; ++i) {
    ...
    #endif
      float depth = 1.0 - texture2D(u_texture_1, vpos * vec2(1, -1) + vec2(0, 1)).r;
    ...
    gl_FragColor = texture2D(u_texture_0, posSum / confidenceSum);
  #endif
